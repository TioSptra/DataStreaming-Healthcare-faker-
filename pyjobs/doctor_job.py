...
SINK_TOPIC = "doctor_data"

def main():
    env_settings = EnvironmentSettings.in_streaming_mode()
    t_env = StreamTableEnvironment.create(environment_settings=env_settings)

    t_env.get_config().set("pipeline.name", "Treatment Tumbling Window Doctor Aggregation")
    ...
    t_env.execute_sql(f"""
        CREATE TABLE rating_doctor_sink (
            window_start        TIMESTAMP(3),
            window_end          TIMESTAMP(3),
            doctor              STRING,
            outcome             STRING,
            total_patient       BIGINT,
            avg_rating          DOUBLE
        ) WITH (
            'connector' = 'kafka',
            'topic' = '{SINK_TOPIC}',
    ...
    t_env.execute_sql("""
        INSERT INTO rating_doctor_sink
        SELECT
            window_start,
            window_end,
            doctor,
